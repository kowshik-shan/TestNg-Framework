...
public class VerifyLoginFunctionality_POM extends BaseTest {
...
@Test
public void VerifyLoginErrorMessage_01(Method mName) throws IOException, InterruptedException {
	...
	loginpage lp = new loginpage(driver);
		...
		sa.assertNotNull(lp.Username.getText(),
			oDataUtils.ReadAccountsProperties("QAaccount.name") );
...
public void VerifyLoginToSalseForce_02(Method mName) throws IOException {
	...
	sa.assertNotNull(lp.Password, oDataUtils.ReadAccountsProperties("QAaccount.password"));
	...
	sa.assertEquals(driver.getCurrentUrl(),
			oDataUtils.ReadPageURLProperties("SalseForce.HomePage") );
	sa.assertAll();
...
public void VerifyCheckRememberMe_03(Method mName) throws IOException, InterruptedException {
	...
	sa.assertNotNull(lp.Username.getText(),oDataUtils.ReadAccountsProperties("QAaccount.name") );
	
	if(waitForElementVisible(lp.Password)) {
		enterText(lp.Password, oDataUtils.ReadAccountsProperties("QAaccount.password"), "Password");
	}
	sa.assertNotNull(lp.Password.getText(), oDataUtils.ReadAccountsProperties("QAaccount.password"));
	...
	Thread.sleep(3000);
	sa.assertEquals(driver.getCurrentUrl(),
			oDataUtils.ReadPageURLProperties("SalseForce.LoginPage") );
	sa.assertEquals(lp.Username.getText(),
			oDataUtils.ReadAccountsProperties("QAaccount.name") );
	sa.assertAll();
}
